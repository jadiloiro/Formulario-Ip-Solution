...
- `login.html` + `login.js`: login form + the forced first-login password-change form. `admin.html` + `admin.js`: `super_admin`-only screen to provision a client account (login/senha/nome + WhatsApp/Telefonia checkboxes), the "campo ADM" — posts to `POST /api/users`.
